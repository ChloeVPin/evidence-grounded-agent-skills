...
class DecisionLedgerTest(unittest.TestCase):
    ...
    def test_dependency_state_diagnostic_snapshot_is_versioned(self):
        ...
        self.assertEqual(len(snapshot["cases"]), 10)
    ...
    def test_snapshot_diagnostic_capture_matches_live_audit(self):
        ...
        snapshot = json.loads((root / capture["snapshot_ref"]).read_text())
        self.assertTrue(validate_snapshot_diagnostic_capture(
            capture, snapshot, {capture["snapshot_ref"]}, {capture["revision"]},
        ).valid)
        self.assertFalse(validate_snapshot_diagnostic_capture(
    ...
    def test_graph_state_diagnostic_capture_matches_live_audit(self):
        root = Path(__file__).resolve().parents[1]
        capture = json.loads((root / "ledger/evidence/0143-graph-state-diagnostic-capture.json").read_text())
        result = subprocess.run(
            ["python3", "scripts/audit_current_assertion.py"],
            cwd=root, capture_output=True, text=True,
        )
    ...
    def test_audit_capture_dependency_summary_is_complete(self):
        ...
        altered_digest = json.loads(json.dumps(summary))
        altered_digest["summary_sha256"] = "0" * 64
        self.assertFalse(validate_audit_capture_dependency_summary(
            altered_digest, available, expected,
        ).valid)

    def test_summary_state_diagnostic_capture_matches_live_audit(self):
        root = Path(__file__).resolve().parents[1]
        capture = json.loads((root / "ledger/evidence/0151-summary-state-diagnostic-capture.json").read_text())
        result = subprocess.run(
            ["python3", "scripts/audit_current_assertion.py"],
            cwd=root, capture_output=True, text=True,
        )
    ...
    def test_executable_current_head_audit_rejects_tampered_freshness_state(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            state["dependency_manifest_ref"] = baseline["dependency_manifest_ref"]
            state["dependency_paths_sha256"] = baseline["dependency_paths_sha256"]
            state["diagnostic_snapshot_sha256"] = "0" * 64
            state_path.write_text(json.dumps(state))
            result = subprocess.run(
                ["python3", "scripts/audit_current_assertion.py", "--root", str(temp_root)],
                cwd=root, capture_output=True, text=True,
            ...
            output = json.loads(result.stdout)
            self.assertIn("self-validation state diagnostic digest is stale", output["reason"])

            state["diagnostic_snapshot_sha256"] = baseline["diagnostic_snapshot_sha256"]
            state["diagnostic_snapshot_ref"] = "ledger/evidence/missing.json"
            state_path.write_text(json.dumps(state))
            result = subprocess.run(
                ["python3", "scripts/audit_current_assertion.py", "--root", str(temp_root)],
                cwd=root, capture_output=True, text=True,
            ...
            state["dependency_graph_ref"] = baseline["dependency_graph_ref"]
            state["dependency_graph_policy_sha256"] = baseline["dependency_graph_policy_sha256"]
            state["capture_summary_sha256"] = "0" * 64
            state_path.write_text(json.dumps(state))
            result = subprocess.run(
                ["python3", "scripts/audit_current_assertion.py", "--root", str(temp_root)],
                cwd=root, capture_output=True, text=True,
            ...
            output = json.loads(result.stdout)
            self.assertIn("self-validation state summary digest is stale", output["reason"])

            state["capture_summary_sha256"] = baseline["capture_summary_sha256"]
            state["capture_summary_ref"] = "ledger/evidence/missing.json"
            state_path.write_text(json.dumps(state))
            result = subprocess.run(
                ["python3", "scripts/audit_current_assertion.py", "--root", str(temp_root)],
                cwd=root, capture_output=True, text=True,
    ...
    def test_executable_current_head_audit_rejects_malformed_versioned_capture(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            shutil.copytree(root / "ledger" / "evidence", temp_root / "ledger" / "evidence")
            shutil.copytree(root / "ledger" / "state", temp_root / "ledger" / "state")
            capture_path = temp_root / "ledger/evidence/0119-four-check-audit-capture.json"
    ...
    def test_executable_current_head_audit_rejects_drifted_dependency_manifest(self):
        root = Path(__file__).resolve().parents[1]
        with tempfile.TemporaryDirectory() as directory:
            temp_root = Path(directory)
            (temp_root / "ledger").mkdir()
            shutil.copytree(root / "ledger" / "evidence", temp_root / "ledger" / "evidence")
            shutil.copytree(root / "ledger" / "state", temp_root / "ledger" / "state")
            manifest_path = temp_root / "ledger/evidence/0125-audit-dependencies.json"
            manifest = json.loads(manifest_path.read_text())
            manifest["paths"].append("ledger/evidence/missing.json")
            ...
            self.assertEqual(output["error_code"], "AUDIT_GATE_FAILED")
            self.assertFalse(output["checks"]["freshness"])
            self.assertIn("audit dependency manifest differs from expected set", output["reason"])

    def test_executable_current_head_audit_rejects_tampered_snapshot_capture(self):
        root = Path(__file__).resolve().parents[1]
        with tempfile.TemporaryDirectory() as directory:
            temp_root = Path(directory)
            (temp_root / "ledger").mkdir()
            shutil.copytree(root / "ledger" / "evidence", temp_root / "ledger" / "evidence")
            shutil.copytree(root / "ledger" / "state", temp_root / "ledger" / "state")
            capture_path = temp_root / "ledger/evidence/0134-snapshot-diagnostic-capture.json"
            capture = json.loads(capture_path.read_text())
    ...
    def test_executable_current_head_audit_rejects_tampered_graph_capture(self):
        root = Path(__file__).resolve().parents[1]
        with tempfile.TemporaryDirectory() as directory:
            temp_root = Path(directory)
            (temp_root / "ledger").mkdir()
            shutil.copytree(root / "ledger" / "evidence", temp_root / "ledger" / "evidence")
            shutil.copytree(root / "ledger" / "state", temp_root / "ledger" / "state")
            capture_path = temp_root / "ledger/evidence/0143-graph-state-diagnostic-capture.json"
            capture = json.loads(capture_path.read_text())
            capture["graph_policy_sha256"] = "0" * 64
            capture_path.write_text(json.dumps(capture))
            result = subprocess.run(
                ["python3", "scripts/audit_current_assertion.py", "--root", str(temp_root)],
                cwd=root, capture_output=True, text=True,
            ...
            self.assertEqual(output["error_code"], "AUDIT_GATE_FAILED")
            self.assertFalse(output["checks"]["freshness"])
            self.assertIn("graph capture policy digest is stale", output["reason"])

    def test_executable_current_head_audit_rejects_drifted_capture_summary(self):
        root = Path(__file__).resolve().parents[1]
        with tempfile.TemporaryDirectory() as directory:
            temp_root = Path(directory)
            (temp_root / "ledger").mkdir()
            shutil.copytree(root / "ledger" / "evidence", temp_root / "ledger" / "evidence")
            shutil.copytree(root / "ledger" / "state", temp_root / "ledger" / "state")
            summary_path = temp_root / "ledger/evidence/0146-audit-capture-dependencies.json"
            summary = json.loads(summary_path.read_text())
            summary["capture_refs"].append("ledger/evidence/missing.json")
    ...
    def test_executable_current_head_audit_rejects_drifted_capture_inventory(self):
        root = Path(__file__).resolve().parents[1]
        with tempfile.TemporaryDirectory() as directory:
            temp_root = Path(directory)
            (temp_root / "ledger").mkdir()
            shutil.copytree(root / "ledger" / "evidence", temp_root / "ledger" / "evidence")
            shutil.copytree(root / "ledger" / "state", temp_root / "ledger" / "state")
            inventory_path = temp_root / "ledger/evidence/0154-freshness-capture-inventory.json"
            inventory = json.loads(inventory_path.read_text())
            inventory["capture_refs"].pop()
            ...
            self.assertEqual(output["error_code"], "AUDIT_GATE_FAILED")
            self.assertFalse(output["checks"]["freshness"])
            self.assertIn("freshness capture inventory captures differ", output["reason"])
