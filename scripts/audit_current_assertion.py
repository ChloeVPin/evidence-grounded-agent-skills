...
def _run(root: Path = ROOT) -> int:
    ...
    graph_provenance_failure = json.loads(
        (evidence_dir / "0174-graph-provenance-state-failure.json").read_text()
    )
    graph_provenance_failure_check = validate_failure_evidence(
        graph_provenance_failure, {
            "ledger/evidence/0151-summary-state-diagnostic-capture.json",
            "ledger/evidence/0154-freshness-capture-inventory.json",
        },
    )
    snapshot_provenance_failure = json.loads(
        (evidence_dir / "0179-snapshot-provenance-state-failure.json").read_text()
    )
    snapshot_provenance_failure_check = validate_failure_evidence(
        snapshot_provenance_failure, {
            "ledger/evidence/0151-summary-state-diagnostic-capture.json",
            "ledger/evidence/0154-freshness-capture-inventory.json",
        },
    )
    ...
    state_failure_state = json.loads(
        (evidence_dir / "0194-state-failure-state-failure.json").read_text()
    )
    state_failure_state_check = validate_failure_evidence(
        state_failure_state, {
            "ledger/evidence/0151-summary-state-diagnostic-capture.json",
            "ledger/evidence/0154-freshness-capture-inventory.json",
        },
    ...
    capture_summary_check = validate_audit_capture_dependency_summary(
        ...
        {path for values in (
            ...
            capture_summary.get("failure_refs", []),
            capture_summary.get("diagnostic_refs", []),
            ...
            capture_summary.get("diagnostic_state_failure_refs", []),
            capture_summary.get("diagnostic_case_refs", []),
    ...
    inventory_available = {
        path for values in (
            ...
            capture_inventory.get("edge_failure_refs", []),
            capture_inventory.get("state_failure_refs", []),
            capture_inventory.get("diagnostic_state_failure_refs", []),
            capture_inventory.get("diagnostic_case_refs", []),
