...
def validate_freshness_capture_inventory(
    ...
    expected_diagnostic_state_failures: set[str] | None = None,
    expected_diagnostic_cases: set[str] | None = None,
) -> ContextAssessment:
    ...
    if expected_state_failures is not None:
        ...
        state_failures = inventory.get("state_failure_refs", [])
    if expected_diagnostic_state_failures is not None:
        diagnostic_state_failures = inventory.get("diagnostic_state_failure_refs")
        if (not isinstance(diagnostic_state_failures, list)
                or set(diagnostic_state_failures) != expected_diagnostic_state_failures
                or len(diagnostic_state_failures) != len(expected_diagnostic_state_failures)):
            return ContextAssessment(False, "freshness capture inventory diagnostic state failures differ")
    else:
        diagnostic_state_failures = inventory.get("diagnostic_state_failure_refs", [])
    if expected_diagnostic_cases is not None:
        diagnostic_cases = inventory.get("diagnostic_case_refs")
        if (not isinstance(diagnostic_cases, list) or set(diagnostic_cases) != expected_diagnostic_cases
                or len(diagnostic_cases) != len(expected_diagnostic_cases)):
            return ContextAssessment(False, "freshness capture inventory diagnostic cases differ")
    else:
        diagnostic_cases = inventory.get("diagnostic_case_refs", [])
    refs = captures + failures + diagnostics + snapshots + edge_failures + state_failures + diagnostic_state_failures + diagnostic_cases + [inventory.get("state_ref"), inventory.get("summary_ref"), inventory.get("graph_ref")]
...
def validate_dependency_diagnostic_snapshot(
    ...
    expected_reasons = {
        ...
        "self-validation state state failure provenance is stale",
        "self-validation state diagnostic state failure provenance is stale",
    }
    reasons = set()
    for case in cases:
...
def validate_self_validation_state(
    ...
    required = (
        ...
        "diagnostic_state_failure_refs", "diagnostic_state_failure_refs_sha256",
        "diagnostic_case_refs", "diagnostic_case_refs_sha256",
    )
    ...
    if capture_inventory is not None:
        ...
        expected_diagnostic_state_failures = capture_inventory.get("diagnostic_state_failure_refs")
        if (state["diagnostic_state_failure_refs"] != expected_diagnostic_state_failures
                or state["diagnostic_state_failure_refs_sha256"] != hashlib.sha256(
                    json.dumps(sorted(expected_diagnostic_state_failures), separators=(",", ":")).encode("utf-8")
                ).hexdigest()):
            return ContextAssessment(False, "self-validation state diagnostic state failure provenance is stale")
        expected_diagnostic_cases = capture_inventory.get("diagnostic_case_refs")
        if (state["diagnostic_case_refs"] != expected_diagnostic_cases
                or state["diagnostic_case_refs_sha256"] != hashlib.sha256(
                    json.dumps(sorted(expected_diagnostic_cases), separators=(",", ":")).encode("utf-8")
                ).hexdigest()):
            return ContextAssessment(False, "self-validation state diagnostic case provenance is stale")
